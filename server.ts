...
const applySeoToHtml = (html: string, seo: { title: string; description: string; canonical: string }) =>
  html
    ...
    .replace(/(<meta name="twitter:url" content=")[^"]*(" \/>)/, `$1${seo.canonical}$2`);

const applyNotFoundSeoToHtml = (html: string, requestPath: string) =>
  html
    .replace(/<title>.*?<\/title>/s, "<title>Page Not Found | ZEECOM Solution</title>")
    .replace(/(<meta name="description" content=")[^"]*(" \/>)/, '$1The requested page could not be found. Return to the ZEECOM Solution homepage or contact our Lahore engineering team.$2')
    .replace(/(<meta name="robots" content=")[^"]*(" \/>)/, '$1noindex, follow$2')
    .replace(/(<link rel="canonical" href=")[^"]*(" \/>)/, `$1https://zeecomsolution.com${requestPath}$2`);
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  const isProductionHost = hostname === "zeecomsolution.com" || hostname === "www.zeecomsolution.com";
  const queryIndex = req.originalUrl.indexOf("?");
  const query = queryIndex >= 0 ? req.originalUrl.slice(queryIndex) : "";

  if (legacyDestination) {
    return res.redirect(301, new URL(legacyDestination, "https://zeecomsolution.com").toString());
  }

  if (isProductionHost && (hostname === "www.zeecomsolution.com" || forwardedProtocol !== "https" || req.path !== normalizedPath)) {
    return res.redirect(301, `https://zeecomsolution.com${normalizedPath}${query}`);
  }

  if (!isProductionHost && req.path !== normalizedPath) return res.redirect(301, `${normalizedPath}${query}`);
  next();
});
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.setHeader("Content-Security-Policy", "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'self'; form-action 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; connect-src 'self'; upgrade-insecure-requests");
...
1. AI Surveillance & Access Control (Intelligent video analytics, biometric facial MFA, unified console, touchless ingress).
...
`;
    ...
    const fallbackData = {
      ...
      voipArchitecture: {
        ...
        crmIntegrationStrategy: "Real-time CRM screen-pop dossier on inbound ring, one-click dialing, and AI conversation speech logging.",
        omnichannelFeatures: [
...
app.post("/api/consultation", async (req: Request, res: Response) => {
  try {
    ...
    if (!resolvedPhone || phoneDigits.length < 7 || phoneDigits.length > 15) {
      return res.status(400).json({
        success: false,
        error: "Please provide a valid telephone number containing 7 to 15 digits.",
      });
    }
    ...
    const leadDirectory = process.env.LEAD_STORAGE_PATH || path.join(process.cwd(), "data");
    fs.mkdirSync(leadDirectory, { recursive: true });
    fs.appendFileSync(path.join(leadDirectory, "consultations.ndjson"), `${JSON.stringify(sanitizedData)}\n`, "utf8");

    if (process.env.CONSULTATION_WEBHOOK_URL) {
      const webhookResponse = await fetch(process.env.CONSULTATION_WEBHOOK_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(sanitizedData),
      });
      if (!webhookResponse.ok) {
        return res.status(502).json({ success: false, error: "Lead was stored locally, but webhook delivery failed. Please call 042-37455670." });
      }
    }

    console.log(`[CONSULTATION_DELIVERED] ID: ${inquiryId} | Timestamp: ${sanitizedData.receivedAt}`);
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get("*", (req: Request, res: Response) => {
      ...
      const seo = LEGAL_PAGE_SEO[requestPath];
      if (requestPath === "/") return res.sendFile(indexPath);
      const html = fs.readFileSync(indexPath, "utf8");
      if (seo) return res.type("html").send(applySeoToHtml(html, seo));
      if (!INDEXABLE_PATHS.has(requestPath)) return res.status(404).type("html").send(applyNotFoundSeoToHtml(html, requestPath));
