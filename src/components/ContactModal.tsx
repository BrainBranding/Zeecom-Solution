...
export const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
  ...
  // Accessible Focus Management, Escape-to-close & Bulletproof Scroll Lock
  useEffect(() => {
    if (!isOpen) return;

    previouslyFocusedElementRef.current = document.activeElement as HTMLElement;

    // Record scroll position and lock body/document scrolling without visual jumps
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Tab' && modalRef.current) {
        const focusableElements = modalRef.current.querySelectorAll<HTMLElement>(
          'button:not([tabindex="-1"]), [href], input:not([tabindex="-1"]), select, textarea, [tabindex]:not([tabindex="-1"])'
        );
    ...
    // Initial focus on the first interactive form input or close button
    const initialFocusTarget = modalRef.current?.querySelector<HTMLElement>('input:not([tabindex="-1"]), button');
    initialFocusTarget?.focus();

    return () => {
      document.removeEventListener('keydown', handleKeyDown);
      // Restore scroll position and unlock document scrolling
        ...
        >
          ...
          {!inquiryResult ? (
            <div>
              ...
              {errorMsg && (
                <div role="alert" aria-live="assertive" className="mb-4 p-3 rounded-xl bg-rose-950/60 border border-rose-500/40 text-rose-200 text-xs sm:text-sm flex items-start gap-2.5">
                  <AlertCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" aria-hidden="true" />
                  <span>{errorMsg}</span>
                </div>
              )}

              {/* Secure POST Form configured with action endpoint */}
              <form
                method="POST"
                ...
                className="space-y-4"
              >
                {/* Spam Protection Honeypot - Visually hidden and completely ignored by assistive technology */}
                <div
                  className="sr-only"
                  aria-hidden="true"
                  style={{
                ...
                >
                  <label htmlFor="form-website-check" aria-hidden="true">Leave this field empty</label>
                  <input
                ...
                {/* Privacy Notice immediately below Submit Button */}
                <div className="pt-2 text-center text-[11px] sm:text-xs text-slate-400 flex items-center justify-center gap-1.5 leading-normal">
                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" aria-hidden="true" />
                  <span>
                    Your details will only be used to respond to your inquiry and will not be shared with third parties. Read our{' '}
                    <a
                      href="/privacy-policy"
                      onClick={() => onClose()}
                      className="text-cyan-400 hover:text-cyan-300 underline underline-offset-2 font-medium transition-colors focus:outline-none focus:ring-1 focus:ring-cyan-400 rounded px-0.5"
                      aria-label="Read ZEECOM Privacy Policy"
                    >
                      Privacy Policy
                    </a>
                    .
            ...
            <div role="status" aria-live="polite" className="py-8 text-center space-y-4">
