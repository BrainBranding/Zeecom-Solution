...
import { Volume2, Megaphone, Radio, Layers, Cpu, CheckCircle, Bell, Music, Flame, Mic, ShieldCheck, Sliders, Play, Square, Activity } from 'lucide-react';
...
export const SolutionAudio: React.FC<SolutionAudioProps> = ({ onOpenConsultation }) => {
  const data = SOLUTIONS_DATA[2]; // IP-Audio

  // Interactive Simulator States
  const [zones, setZones] = useState<AudioZone[]>(MOCK_AUDIO_ZONES);
  const [selectedZoneIds, setSelectedZoneIds] = useState<string[]>(['ZONE-1', 'ZONE-2']);
  const [isBroadcasting, setIsBroadcasting] = useState(false);
  const [isEmergencyActive, setIsEmergencyActive] = useState(false);
  const [isBgmActive, setIsBgmActive] = useState(true);
  const [broadcastMessage, setBroadcastMessage] = useState('All staff: Fire drill briefing at 14:00 in Auditorium B.');
  ...
  // Trigger Emergency Evacuation Alert
  const handleEmergencyAlert = () => {
    soundFx.playEmergencySiren();
    setIsEmergencyActive(true);
    // Set all zones to Emergency
    setZones(prev => prev.map(z => ({ ...z, status: 'EMERGENCY_ALERT' })));
  };
  ...
  const handleVolumeChange = (id: string, vol: number) => {
    setZones(prev => prev.map(z => z.id === id ? { ...z, volume: vol } : z));
  };

  return (
    <section id="ip-audio" className="py-20 bg-slate-950 text-slate-100 border-t border-slate-800 relative overflow-hidden">
      {/* Background glow */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-lg text-amber-300 font-medium">
            {data.tagline}
          </p>
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <div className="p-2.5 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20">
                    {idx === 0 && <Megaphone className="w-5 h-5" />}
        ...
        {/* ========================================================================= */}
        {/* INTERACTIVE IP-AUDIO MULTI-ZONE PA BROADCAST SIMULATOR */}
        {/* ========================================================================= */}
        <div className="mt-16 p-6 sm:p-8 rounded-2xl bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950 border border-amber-500/30 shadow-2xl">
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-800">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs transition-colors flex items-center gap-1.5 shadow-lg shadow-amber-500/20"
              >
              ...
              {!isEmergencyActive ? (
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-red-600/20 hover:bg-red-600/30 text-red-300 border border-red-500/40 text-xs font-bold transition-colors flex items-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-red-600 text-white font-bold text-xs animate-pulse flex items-center gap-1.5 shadow-lg shadow-red-600/30"
                >
          ...
          {isEmergencyActive && (
            <div className="mt-4 p-3 rounded-xl bg-red-950/80 border border-red-500 text-red-200 text-xs font-bold flex items-center justify-between animate-pulse">
              <div className="flex items-center gap-2">
                <Flame className="w-4 h-4 text-red-400" />
                <span>CRITICAL EVACUATION OVERRIDE ENGAGED ACROSS ALL 250+ ADDRESSABLE ZONES. 123dB HIGH-OUTPUT ACTIVE.</span>
              </div>
              <span className="px-2 py-0.5 rounded bg-red-800 text-white text-[10px]">ALL ZONES OVERRIDDEN</span>
            </div>
          )}

          {/* Broadcasting Live Audio Banner */}
          {isBroadcasting && (
            <div className="mt-4 p-3 rounded-xl bg-amber-950/60 border border-amber-500 text-amber-200 text-xs font-semibold flex items-center justify-between animate-pulse">
              <div className="flex items-center gap-2">
          ...
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 space-y-3">
              <div className="flex items-center justify-between pb-2">
                <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                  Addressable Facility Audio Zones ({zones.length} shown / 250+ capacity)
                </span>
                <div className="flex items-center gap-2">
                  <button
            ...
            <div className="lg:col-span-5 p-5 rounded-xl bg-slate-950 border border-slate-800 flex flex-col justify-between space-y-4">
              <div>
                ...
                <div className="mt-5 space-y-2 pt-4 border-t border-slate-800 text-xs">
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-slate-900/60 border border-slate-800">
                    ...
                    <span className="text-cyan-300 font-semibold font-mono">Standard CAT6 PoE (No 70V)</span>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800 flex flex-col justify-between">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {data.idealFor.map((item, i) => (
                  <div key={i} className="p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-xs font-medium text-slate-300 flex items-center gap-2">
            ...
            <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between">
              <span className="text-xs text-slate-400">Need acoustic modeling for your facility?</span>
              <button
                onClick={onOpenConsultation}
                className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs transition-colors shadow-lg shadow-amber-500/20"
              >
