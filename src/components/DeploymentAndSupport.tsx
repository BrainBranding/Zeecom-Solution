...
export const DeploymentAndSupport: React.FC<DeploymentAndSupportProps> = ({ onOpenConsultation }) => {
  ...
  return (
    <section id="deployment" className="py-14 sm:py-18 bg-slate-900/50 text-slate-100 border-t border-slate-800 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 3 Deployment Models - Single column on mobile, 3-col on desktop */}
          ...
          {COMPANY_INFO.deploymentOptions.map((opt, idx) => (
            ...
            >
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed mb-4">{opt.desc}</p>
        ...
        {/* 4 Comprehensive Support Pillars - 1-col on mobile, 2-col on tablet, 4-col on desktop */}
        ...
        <div className="mt-14 sm:mt-16 p-5 sm:p-8 rounded-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border border-slate-800 shadow-2xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
            <div className="lg:col-span-6 space-y-4">
              ...
              <div>
                <label htmlFor="security-audit-setup" className="text-xs sm:text-sm font-semibold text-slate-200 block mb-1.5">
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-xl bg-slate-900 border border-slate-800 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-cyan-500 resize-none min-h-[80px]"
                />
              </div>
              ...
              <button
                ...
                className="w-full sm:w-auto min-h-[44px] px-5 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs sm:text-sm shadow-lg shadow-cyan-500/20 transition-colors flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-cyan-400"
              >
                {isAuditing ? (
                  <>
            ...
            {/* Audit Output Result */}
            <div aria-live="polite" aria-busy={isAuditing} className="lg:col-span-6 p-4 sm:p-5 rounded-xl bg-slate-950 border border-slate-800">
              {auditResult ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                    <span className="text-xs sm:text-sm font-bold text-white">Audit Assessment Report</span>
                    <span className="text-xs sm:text-sm font-mono font-bold px-2 py-0.5 rounded bg-amber-500/20 text-amber-300">
                      Readiness Score: {auditResult.readinessScore}/100
                    </span>
                  </div>

                  <div>
                    <span className="text-xs font-bold text-red-400 block mb-1">Critical Vulnerabilities in Legacy Setup:</span>
                  ...
                  <div className="pt-2 border-t border-slate-800/80">
                    <span className="text-xs font-bold text-cyan-400 block mb-1">Recommended ZEECOM Upgrade Path:</span>
                    <ul className="space-y-1.5 text-xs sm:text-sm text-slate-300">
                  ...
                  <div className="p-3 rounded-lg bg-slate-900 border border-slate-800 text-xs sm:text-sm text-slate-300">
                ...
                <div className="text-center py-8">
                  ...
                  <span className="text-xs sm:text-sm text-slate-400 block">Click "Run AI Security Readiness Check" to analyze legacy risks.</span>
                </div>
              )}
            </div>
          </div>
